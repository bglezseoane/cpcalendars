...
fn main() {
    ...
    // Write a info file inside the ICBU
    let info = "This ICBU is not generated by Calendars.app. It is a 'cpcalendars' rebuild. \
        Normally here also appears an 'Info.plist' file.\n";
    match fs::write(&composed_destination.join("Info.txt"), &info) {
        Err(e) => {
            println!("Error: {}", e);
            return;
        }
        Ok(_f) => println!("Process finished successfully."),
    }
}
